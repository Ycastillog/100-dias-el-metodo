...
for (const path of ['/assets/app.js', '/assets/ops.js', '/assets/life-program.js', '/hosting/curriculum-lessons.json', '/assets/curriculum-lessons.json', '/hosting/curriculum.mjs', '/hosting/local-qa.mjs', '/__qa/mobile', '/.env', '/.git/config']) assert.equal((await worker.fetch(new Request(origin + path), env)).status, 404);
assert.doesNotMatch(await readFile(new URL('../dist/server/index.js',import.meta.url),'utf8'),/METODO_LOCAL_QA|LOCALTEST|LOCAL-QA/);
for(const path of ['/assets/toolkit.js','/assets/welcome.js'])assert.equal((await worker.fetch(new Request(origin+path),env)).status,200);
